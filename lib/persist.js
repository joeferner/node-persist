...
exports.defineAuto = function(name, options, callback) {
  ...
  var callbackFcn       = callback;

  if ( arguments.length < 3 && databaseJsonLoaded ) {
    switch (typeof name) {
        case 'string':
          modelName = name;
          break;
        ...
        case 'function':
        default:
          throw new TypeError('Error must define model name as a string.');
    }
    ...
    switch (typeof options) {
      ...
      case 'function':
        callbackFcn       = options;
        connectionOptions = getDefaultConnectOptions();
        break;
      default:
