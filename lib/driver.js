...
var Driver = persistUtils.Class.extend({
  ...
  getInsertSql: function(obj) {
    ...
    var sql = util.format('INSERT INTO %s (%s) VALUES (%s);', obj._model().tableName, columnNamesSql.join(','), valuesSql.join(','));
  ...
  getUpdateSql: function(obj) {
    ...
    var sql = util.format('UPDATE %s SET %s WHERE %s = %s;', obj._model().tableName, columnNamesSql.join(','), idColumndName, this.getValuesSubstitutionString(valueSubstitutionIndex++));
    //console.log(sql);
    ...
    return expr.replace(/([a-zA-Z_\.]+)/g, function(m) {
      ...
      if(parts.length == 1) {
        if(sqlTree.model && sqlTree.model.columns) {
          var column = sqlTree.model.columns[m];
          if(column) {
        ...
        throw new Error("Nested where clauses not handled '" + m + "'");
      }
    });
  },
