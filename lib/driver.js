...
var Driver = persistUtils.Class.extend({
  ...
  getUpdateSql: function(obj) {
    ...
    var idColumndName = obj._getModel().getIdPropertyName();
    values.push(obj.getId());

    var sql = util.format('UPDATE %s SET %s WHERE %s = %s', obj._getModel().tableName, columnNamesSql.join(','), idColumndName, this.getValuesSubstitutionString(valueSubstitutionIndex++));
    //console.log(sql);
