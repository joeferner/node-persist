...
function createColumnPropertiesOnObject(obj) {
  for(var column in obj._model.columns) {
    var setter = obj.__lookupSetter__(column);
    if(!obj[column]) {
      if(setter) {
        if(!obj._model.columns[column].foreignKey) {
          setter(null);
        }
      } else {
        obj[column] = null;
...
function hasMany(model) {
  ...
  var foreignKeyName = persistUtils.toPropertyName(this.modelName + 'Id');
