...
function saveInstance(connection, callback) {
  ...
  this._getModel().emit("beforeSave", this);
  if(this._isPersisted()) {
    self._getModel().emit("beforeUpdate", self);
    connection.update(this, function() {
      self._getModel().emit("afterUpdate", self);
      self._getModel().emit("afterSave", self);
      callback.apply(self, arguments);
    });
  } else {
    this._getModel().emit("beforeCreate", this);
    connection.save(this, function() {
      self._getModel().emit("afterCreate", self);
      self._getModel().emit("afterSave", self);
      callback.apply(self, arguments);
    });
  }
