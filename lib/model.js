...
function addColumn(propertyName, columnDef) {
  var col = normalizeColumnDef(propertyName, columnDef);
...
function hasMany(associatedModel) {
  ...
  this.associations[name] = { type: "hasMany", model: associatedModel, foreignKey: foreignKey };

  name = inflection.camelize(this.modelName, true);
  if(!associatedModel.associations[name]) {
    associatedModel.associations[name] = { type: "hasOne", model: this, foreignKey: foreignKey };
  }
  var foreignKeyPropertyName = inflection.camelize(foreignKey, true);
  associatedModel.addColumn(foreignKeyPropertyName, { type: "int", foreignKey: true });

  return this;
};

function hasOne(associatedModel) {
  var foreignKey = inflection.foreignKey(this.modelName);
  var name = inflection.camelize(associatedModel.modelName, true);
  this.associations[name] = { type: "hasOne", model: associatedModel, foreignKey: foreignKey };

  name = inflection.camelize(inflection.pluralize(this.modelName), true);
  if(!associatedModel.associations[name]) {
    associatedModel.associations[name] = { type: "hasMany", model: this, foreignKey: foreignKey };
  }
...
function orderBy(arg, callback) {
  return this.using(null).orderBy(arg, callback);
}

function all(connection, callback) {
  return this.using(connection).all(callback);
}

function first(connection, callback) {
  return this.using(connection).first(callback);
}

function deleteAll(connection, callback) {
  return this.using(connection).deleteAll(callback);
}

function min(fieldName) {
  return this.using(null).min(fieldName);
}

function max(fieldName) {
  return this.using(null).max(fieldName);
}
